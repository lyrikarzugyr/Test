...
deploy_streamrnode() {
		...
		do
				...
				expect << EOF
				set timeout 60
				spawn docker run -it -v $(cd $HOME/.streamrDocker_${i}; pwd):/root/.streamr streamr/broker-node:latest bin/config-wizard
...
EOF
				...
				docker create --name streamr_${i} --restart=always -p ${websocket_port}:${websocket_port} -p ${publishHttp_port}:${publishHttp_port} -p ${mqtt_port}:${mqtt_port} -v $(cd $HOME/.streamrDocker_${i}; pwd):/root/.streamr streamr/broker-node:latest
