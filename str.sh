...
deploy_streamrnode() {
		...
		do
				...
				send "a\r"
				expect "Select a port for the websocket Plugin" 
				send "$websocket_port\r"
				expect "Select a port for the mqtt Plugin" {send "$mqtt_port\r"}
				expect "Select a port for the publishHttp Plugin" {send "$publishHttp_port\r"}
